      ...
      meta: [
        {
          ...
          name: "description",
          content:
            "Toutes les décisions de l'autorité de l'aviation civile de djibouti",
        },
      ],
    };
  },
    ...
    sortedDocuments() {
      if (this.documents.length > 0) {
        let sorted = this.documents.filter((doc) => {
          return doc.idParent == "5f53709c09b8d5003da4daf8";
        });
        return sorted;
      } else {
        let sorted = this.documents.filter((doc) => {
  ...
  methods: {
    ...
    fetchBreadCrumb() {
      this.$store.state.category.subCategoryOne.forEach((subOne) => {
        if (subOne._id === "5f53709c09b8d5003da4daf8") {
          this.subOne = subOne;
    ...
    fetchMenu() {
      ...
      const subOne = this.$store.state.category.subCategoryOne.filter(
        (element) => {
          return element._id === "5f53709c09b8d5003da4daf8";
        }
      );
