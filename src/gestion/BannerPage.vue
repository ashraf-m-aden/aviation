<template>
  <div class="banners">
    ...
    <p v-if="!banniere.length" class="banners__empty">
    ...
    <div v-else class="bgrid">
      <div
        class="bcard"
        v-for="(banner, index) in banniere"
      ...
      >
        ...
        <div class="bcard__media">
          ...
          <input
            ...
            "
            @input="setImage(banner, $event.target.value)"
          />
          <span class="imgid" v-if="banner.driveId"
            >id : {{ banner.driveId }}</span
          >
        </div>
        ...
        <div class="bcard__body">
          ...
          <div class="bcard__foot">
            <div class="order">
              ...
              <button
                class="ico"
                :disabled="index === banniere.length - 1"
                @click="down(index)"
                title="Descendre"
              >
                ▼
            ...
            <div class="acts">
              <button class="btn-save" @click="modify(banner)">Modifier</button>
              <button class="ico del" @click="remove(banner)" title="Supprimer">
                🗑
              </button>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    // remplace l'image : on extrait le nouvel ID Drive, l'aperçu se met à jour.
    // Pense à cliquer « Enregistrer » pour persister.
    setImage(item, link) {
      const id = extractDriveId(link);
      if (id) item.driveId = id;
    },
    modify(banner) {
